...
**Phase 2 — design captured, implementation pending.** transformers 5.8.1 has the DSv4 architecture but no MTP module class. Chosen approach: vendor `inference/model.py` + `kernel.py` from upstream (already copied to `vendor/dsv4-upstream/`) and adapt — swap their FP-aware `Linear` for `nn.Linear` so GPTQ hooks fire natively, replace their custom kernels with PyTorch reference impls for calibration (sparse_attn → SDPA, hc_split_sinkhorn → hand-rolled, act_quant → no-op since W4A16 doesn't quantize activations). See [`PHASE2_DESIGN.md`](PHASE2_DESIGN.md) for the full design + the 7-step plan for next session.
