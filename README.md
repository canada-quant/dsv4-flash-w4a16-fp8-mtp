...
**Phase 1 — dequant running.** EC2 box (`i-0714f36a266c8c59b`, `us-west-2`, 8× B300) bootstrapped; `/data` mounted (300 GB EBS, persistent), `/scratch` symlinked to instance-store NVMe (27.6 TB). `venv-calib` built and patched: `transformers==5.8.1`, `compressed-tensors==0.15.1a20260515` (alpha — the predecessor's "phantom" pin shipped on 2026-05-15), `llm-compressor@f2aa32e2`, `torch 2.12.0+cu130`. Both transformers hunks (mtp-retention + calibration cache) and the llm-compressor helpers hunk are applied; runtime asserts that `DeepseekV4PreTrainedModel._keys_to_ignore_on_load_unexpected == []`. Upstream weights (149 GB / 69,187 tensors / 46 shards) downloaded from `deepseek-ai/DeepSeek-V4-Flash`. Upstream confirmed to ship 1,575 `mtp.0.*` tensors in shard 46 — naming is DeepSeek internal convention (`attn.wq_a` not HF `self_attn.q_a_proj`, scale suffix `.scale` not `.weight_scale_inv`); `dequant_mtp.py` rewritten to match. Dequant currently writing BF16 to `/scratch/weights/bf16-mtp` (~400 GB expected output).

**Phase 2 — blocked on MTP shim.** transformers 5.8.1 has the DSv4 architecture but no MTP module class — `from_pretrained` cannot attach the deserialized mtp.* tensors to any `nn.Module`, so GPTQ via llmcompressor cannot see them. Plan: define a `DeepSeekV4MTPLayer` shim inside `scripts/quantize_v4_w4a16_mtp.py` driven by the upstream `inference/model.py` wiring. See `PLAN.md` Phase 2 note + the script's docstring for the open design questions.
